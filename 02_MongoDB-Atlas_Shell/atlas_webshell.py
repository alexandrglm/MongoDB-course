############################################################################
# PyMongoDB Atlas WebShell v.0.1 (Standalone)
################################
import os
...
def render_project12():
    return render_template('PyMongoDB_Atlas_Shell.html')
...
class MenuComunes:
    ...
    def comunes(self, command):
        if command == 'menu':
            return {
                'output': self.menu_manager.get_menu(),
                'raw_output': ''
            }
        elif command == 'clear':
            return {
...
############################################################################
# Menu! Pending a lot of fixes (Delete sequential logic, data inputs, ...)
################################
...
def socketio_commander(data):
    ...
    try:
        # Manejar comandos comunes (menu, clear, help, exit)
        response = menu_comunes.comunes(command)
        if response:
            emit('execute_command_response', response)
            return

        # Lógica específica del menú actual
